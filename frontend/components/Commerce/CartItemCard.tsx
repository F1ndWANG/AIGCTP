...
export default function CartItemCard({ item, onUpdateQuantity, onRemove }: CartItemCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg border dark:border-slate-700 p-4 flex items-center gap-4">
      <div className="w-16 h-16 bg-gray-50 dark:bg-slate-900 rounded-lg flex items-center justify-center flex-shrink-0">
        {item.product_image ? (
      ...
      <div className="flex-1 min-w-0">
        <h4 className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{item.product_name}</h4>
        {Object.keys(item.specs).length > 0 && (
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
            {Object.entries(item.specs).map(([k, v]) => `${k}: ${v}`).join(", ")}
          </p>
        )}
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">¥{item.price}/{item.unit}</p>
