...
async def retry_task(
    ...
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    if task.task_type not in ("chat", "chat_stream", "chat_background"):
        raise HTTPException(status_code=400, detail="Only chat tasks can be retried")

    if task.task_type == "chat_background":
        from app.services.chat_orchestrator import enqueue_chat_background as _enqueue_chat_background
        ...
        return await _enqueue_chat_background(
            ChatRequest(
                message=message,
